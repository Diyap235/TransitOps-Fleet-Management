...
const Modal = ({ title, onClose, onSubmit, form, setForm, loading, error, vehicles, isEdit }) => (
  <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: 20 }}>
    <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 500, boxShadow: '0 24px 64px rgba(0,0,0,0.18)' }}>
...
const Maintenance = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) await updateMaintenanceRecord(editing, form);
      else await createMaintenanceRecord(form);
  ...
  const handleClose = async (id) => {
    try { await updateMaintenanceRecord(id, { status: 'Closed' }); load(); }
    catch (err) { alert(err.response?.data?.message || 'Failed to close record'); }
  };

  const handleDelete = async (id) => {
    if (!window.confirm('Delete this maintenance record?')) return;
    try { await deleteMaintenanceRecord(id); load(); } catch (err) { alert(err.response?.data?.message || 'Delete failed'); }
  };
  ...
  return (
    <>
      {modalOpen && <Modal title={editing ? 'Edit Record' : 'Log Maintenance'} onClose={() => setModalOpen(false)} onSubmit={handleSubmit} form={form} setForm={setForm} loading={saving} error={error} vehicles={vehicles} isEdit={!!editing} />}

      <div className="page-header">
        <div className="page-header-left"><h1>Maintenance</h1><p>Track vehicle service and repair history.</p></div>
