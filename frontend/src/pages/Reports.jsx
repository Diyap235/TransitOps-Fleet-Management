...
import {
  Download, TrendingUp, TrendingDown, Gauge, Fuel, DollarSign,
  BarChart2, AlertCircle, FileText, FileType,
} from 'lucide-react';
import {
  AreaChart, Area,
  BarChart, Bar,
...
// ─── Dummy data (overview charts) ──────────────────────────────────────────

const fleetUtilizationData = [
  { month: 'Jan', utilization: 68 }, { month: 'Feb', utilization: 72 },
  { month: 'Mar', utilization: 75 }, { month: 'Apr', utilization: 70 },
  { month: 'May', utilization: 78 }, { month: 'Jun', utilization: 74 },
  { month: 'Jul', utilization: 80 }, { month: 'Aug', utilization: 77 },
  { month: 'Sep', utilization: 82 }, { month: 'Oct', utilization: 79 },
  { month: 'Nov', utilization: 83 }, { month: 'Dec', utilization: 82 },
];

const fuelEfficiencyData = [
  { month: 'Jan', efficiency: 5.8 }, { month: 'Feb', efficiency: 5.9 },
  { month: 'Mar', efficiency: 6.0 }, { month: 'Apr', efficiency: 5.7 },
  { month: 'May', efficiency: 6.1 }, { month: 'Jun', efficiency: 6.2 },
  { month: 'Jul', efficiency: 6.0 }, { month: 'Aug', efficiency: 6.3 },
  { month: 'Sep', efficiency: 6.1 }, { month: 'Oct', efficiency: 6.4 },
  { month: 'Nov', efficiency: 6.3 }, { month: 'Dec', efficiency: 6.4 },
];

const vehicleRoiData = [
  { month: 'Jan', roi: 14.2 }, { month: 'Feb', roi: 14.8 },
  { month: 'Mar', roi: 15.1 }, { month: 'Apr', roi: 14.5 },
  { month: 'May', roi: 15.6 }, { month: 'Jun', roi: 15.9 },
  { month: 'Jul', roi: 16.0 }, { month: 'Aug', roi: 16.4 },
  { month: 'Sep', roi: 16.8 }, { month: 'Oct', roi: 16.5 },
  { month: 'Nov', roi: 17.0 }, { month: 'Dec', roi: 17.2 },
];

const costTrendsData = [
  { month: 'Jan', costPerKm: 0.51 }, { month: 'Feb', costPerKm: 0.49 },
  { month: 'Mar', costPerKm: 0.48 }, { month: 'Apr', costPerKm: 0.50 },
  { month: 'May', costPerKm: 0.47 }, { month: 'Jun', costPerKm: 0.46 },
  { month: 'Jul', costPerKm: 0.47 }, { month: 'Aug', costPerKm: 0.45 },
  { month: 'Sep', costPerKm: 0.44 }, { month: 'Oct', costPerKm: 0.43 },
  { month: 'Nov', costPerKm: 0.44 }, { month: 'Dec', costPerKm: 0.42 },
];

// ─── KPI cards config ───────────────────────────────────────────────────────

const KPI_CARDS = [
  { label: 'Fleet Utilization', value: '82%', trend: '+5.1%', up: true, icon: Gauge, iconBg: '#EFF6FF', iconColor: '#2563EB', chartId: 'fleet-utilization' },
  { label: 'Fuel Efficiency', value: '6.4 km/L', trend: '+1.9%', up: true, icon: Fuel, iconBg: '#EFF6FF', iconColor: '#0EA5E9', chartId: 'fuel-efficiency' },
  { label: 'Vehicle ROI', value: '17.2%', trend: '+1.4%', up: true, icon: BarChart2, iconBg: '#F5F3FF', iconColor: '#7C3AED', chartId: 'vehicle-roi' },
  { label: 'Cost per KM', value: '₹0.42', trend: '-2.3%', up: false, icon: DollarSign, iconBg: '#EFF6FF', iconColor: '#06B6D4', chartId: 'cost-trends' },
];
...
// ─── Overview export helpers (TXT / print-to-PDF) ──────────────────────────
...
const Reports = () => {
  ...
  const handleExport = (type) => {
    try {
      if (type === 'fuel') exportFuelLogsCSV(fuelLogs);
      else if (type === 'expenses') exportExpensesCSV(expenses);
      else if (type === 'trips') exportTripSummaryCSV(trips);
    } catch (err) {
    ...
    <>
      ...
      {error && (
        <div className="alert alert-error" style={{ marginBottom: 20 }}>
          <AlertCircle size={16} />
          {error}
          <button className="btn btn-ghost btn-sm" onClick={fetchReportData} style={{ marginLeft: 'auto' }}>
            Retry
          </button>
        </div>
      )}
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 10 }}>
          ...
          {activeTab === 'overview' && (
            <div style={{ position: 'relative' }}>
              ...
              {showFormatMenu && (
                <>
                  <div style={{ position: 'fixed', inset: 0, zIndex: 98 }} onClick={() => setShowFormatMenu(false)} />
                  <div style={{
                    ...
                    <button
                      onClick={() => { exportAsPdf(); setShowFormatMenu(false); }}
          ...
          {!hasReportData ? (
            <div className="card" style={{ marginBottom: 20 }}>
              <div className="empty-state">
                <div className="empty-state-icon"><BarChart2 size={22} strokeWidth={1.5} /></div>
              ...
              <div className="stats-grid" style={{ marginBottom: 28 }}>
                {KPI_CARDS.map(({ label, value, trend, up, icon: Icon, iconBg, iconColor, chartId }) => (
                  <div
                    ...
                    onMouseLeave={e => { e.currentTarget.style.transform = highlightedChart === chartId ? 'scale(1.02)' : 'scale(1)'; }}
                  >
                    <div className="stat-card-header">
                      ...
                      <span
                        ...
                        style={{ fontSize: 12, fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 3 }}
                      >
              ...
              <div className="card" style={{ marginBottom: 20 }}>
                ...
                <div className="card-body">
                      ...
                      return (
                        <div key={day} style={{ textAlign: 'center' }}>
                          <div style={{
                            ...
                            <div style={{
                              width: '100%', height: `${utilization}%`,
                              background: `linear-gradient(180deg, ${color}80, ${color}40)`,
                              borderRadius: 'var(--radius-sm) var(--radius-sm) 0 0',
                            }} />
                            <div style={{ position: 'absolute', top: 4, left: 0, right: 0, fontSize: 12, fontWeight: 700, color }}>
                              {utilization}%
                            </div>
                          </div>
                  ...
                  <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 20, fontSize: 12 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 12, height: 12, background: '#D97706', borderRadius: 3 }} />
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                      <AreaChart data={fleetUtilizationData} margin={{ top: 4, right: 16, left: -16, bottom: 0 }}>
                        ...
                        <Area
                          ...
                          stroke="#2563EB" strokeWidth={2.5} fill="url(#gradUtil)"
                  ...
                  <div className="card-body" style={{ padding: '16px 8px 8px' }}>
                    <ResponsiveContainer width="100%" height={220}>
                      <BarChart data={fuelEfficiencyData} margin={{ top: 4, right: 16, left: -16, bottom: 0 }} barSize={18}>
                        ...
                        <Bar dataKey="efficiency" name="Efficiency" fill="#06B6D4" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                  ...
                  <div className="card-body" style={{ padding: '16px 8px 8px' }}>
                    <ResponsiveContainer width="100%" height={220}>
                      <LineChart data={costTrendsData} margin={{ top: 4, right: 16, left: -16, bottom: 0 }}>
                        ...
                        <Line
                          type="monotone" dataKey="costPerKm" name="Cost/km"
